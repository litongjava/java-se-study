...
public class EasyExcelDemo {
  public static void main(String[] args) throws IOException {
    ...
    //demo.writeExcelOneSheetOnceWrite();
    demo.writeExcelOneSheetMoreWrite();
  }
  ...
  public void writeExcelOneSheetOnceWrite() throws IOException {
    ...
    // 生成EXCEL并指定输出路径
    OutputStream out = new FileOutputStream("withoutHead1.xlsx");
  ...
  /**
   * 针对105W以内的记录数可以调用该方法分多批次查出然后写入到EXCEL的一个SHEET中 <br>
   * 注意：每次查询出来的记录数量不宜过大，根据内存大小设置合理的每次查询记录数，不会内存溢出即可。 数据量不能超过一个SHEET存储的最大数据量105W
   *
   * @throws IOException
   */
  public void writeExcelOneSheetMoreWrite() throws IOException {
    ...
    // 生成EXCEL并指定输出路径
    OutputStream out = new FileOutputStream("withoutHead2.xlsx");
    ExcelWriter writer = new ExcelWriter(out, ExcelTypeEnum.XLSX);
    ...
    /**
     * 注： 此处仅仅为了模拟数据，实用环境不需要将最后一次分开，合成一个即可， <br>
    ...
    for (int i = 0; i < writeCount; i++) {
      ...
      if (i < writeCount - 1) {
        ...
        for (int j = 0; j < pageSize; j++) {
          userList
              .add(Arrays.asList("ID_" + Math.random(), "小明", String.valueOf(Math.random()), new Date().toString()));
        }
        writer.write0(userList, sheet, table);
  ...
  /**
   * 针对几百万的记录数可以调用该方法分多批次查出然后写入到EXCEL的多个SHEET中<br>
  ...
  public void writeExcelMoreSheetMoreWrite() throws IOException {
    ...
    // 模拟分批查询：总记录数250条，每个SHEET存100条，每次查询20条 则生成3个SHEET，前俩个SHEET查询次数为5，
    // 最后一个SHEET查询次数为3 最后一次写的记录数是10
    // 注：该版本为了较少数据判断的复杂度，暂时perSheetRowCount要能够整除pageSize， 不去做过多处理
    // 合理分配查询数据量大小不会内存溢出即可。
    ...
    Integer lastSheetWriteCount = totalRowCount % perSheetRowCount == 0 ? previousSheetWriteCount
        : (totalRowCount % perSheetRowCount % pageSize == 0 ? totalRowCount % perSheetRowCount / pageSize
    ...
    for (int i = 0; i < sheetCount; i++) {
      ...
      if (i < sheetCount - 1) {
        ...
        // 前2个SHEET, 每个SHEET查5次 每次查20条 每个SHEET写满100行 2个SHEET合计200行 实用环境：参数：
        // currentPage: j+1 + previousSheetWriteCount*i, pageSize: pageSize
        ...
        // 最后一个SHEET 实用环境不需要将最后一次分开，合成一个即可， 参数为： currentPage = i+1; pageSize =
        // pageSize
        for (int j = 0; j < lastSheetWriteCount; j++) {
          ...
          if (j < lastSheetWriteCount - 1) {
            ...
            for (int k = 0; k < 20; k++) {
              userList.add(
                  Arrays.asList("ID_" + Math.random(), "小明", String.valueOf(Math.random()), new Date().toString()));
            }
            writer.write0(userList, sheet, table);
            ...
            for (int k = 0; k < lastWriteRowCount; k++) {
              userList.add(
                  Arrays.asList("ID_" + Math.random(), "小明1", String.valueOf(Math.random()), new Date().toString()));
            }
            writer.write0(userList, sheet, table);
